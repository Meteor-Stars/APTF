...
class DSAutoCorrelation(nn.Module):
    ...
    def forward(self, queries, keys, values, attn_mask, tau=None, delta=None):
        ...
        # if corr.shape[-1]!=delta.shape[-1]:
        #     zeros=torch.zeros((corr.shape[0],corr.shape[1],corr.shape[2],1)).to(corr.device)
        #     corr=torch.cat([corr,zeros],dim=-1)
        # # print(corr.shape,tau.shape,delta.shape)
        # else:
        #     corr = corr * tau + delta
