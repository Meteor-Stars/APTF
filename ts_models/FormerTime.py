...
class Encoder(nn.Module):
    ...
    def __init__(self, slice_size, data_shape, d_encoder, attn_heads, enable_res_parameter, device, tr,
        ...
        # print('*****')
        # print(self.slice_size,d_encoder) #(16, 9) 64
    ...
    def forward(self, x):
        if len(x.shape) == 4:
            x = x.squeeze(1)
        # print(x.shape) #torch.Size([64, 144, 9])

        x = self.input_projection(x.transpose(1, 2)).transpose(1, 2)
        x = self.input_norm(x)
        # print(x.shape) #torch.Size([64, 17, 64])
...
class FormerTime(nn.Module):
    ...
    def __init__(self, args):
        ...
        # print(self.slice_sizes) #[(16, 9), (2, 64), (2, 64)]
    ...
    def forward(self, x):
        # if x.shape[1]!=self.args.var_num:
        #     x=x.transpose(1,2)
        # print(x.shape)
        # print('3333')
        # time.sleep(500)
