...
class OmniScaleCNN(nn.Module):
    def __init__(self, c_in, seq_len, c_out,layers=[8 * 128, 5 * 128 * 256+ 2 * 256 * 128 ], few_shot=False,args=None):
        super(OmniScaleCNN, self).__init__()
        #
        # args.log_.log('Mddel c_in {} c_out {} seq_len {}'.format(c_in, c_out, seq_len))
        # print(seq_len)
        # time.sleep(500)
        ...
        print('out_put_channel_number',out_put_channel_number)
        # self.hidden = nn.Linear(out_put_channel_number, args.num_class)
    def forward(self, x):
        if len(x.shape)!=3:
            x=x.unsqueeze(1)
        ...
        # print(x.shape)
        x = self.net(x)
        # print(x.shape) #torch.Size([64, 44, 315])
        # time.sleep(500)
        # x=self.hidden(x.transpose(1,2)).transpose(1,2)

        x = self.gap(x)
        # x=x.mean(-1)
        # print(x.shape) #torch.Size([64, 44])
        # time.sleep(500)
        x=self.hidden(x)
        # print(x.shape)
        # time.sleep(500)
